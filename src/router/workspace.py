...
def GetWorkspaceByName(workspace_location : str, db : Session = Depends(get_db)):
    try:
        logger.info(f"Getting workspace with Name : {workspace_location}")
...
@router.get('/get_all_workspaces', response_model=WorkspaceResponse)
def GetWorkspaceByName(db : Session = Depends(get_db)):
    try:
